...
- ~~接通附件按钮，支持图片和文件选择、预览、移除，并按 Harness 能力传递。~~ **已完成**：原生对话框选件（图片 ≤10MB base64 / 文本 ≤200KB 内联进 prompt），composer 顶部 chips 预览与移除；图片走四家原生协议（Pi `images` 字段、Claude 内容块、DSH `PromptContentPart`、Codex `UserInput image`），manifest 新增 `attachments` 能力位（四家均已声明）；已发送附件在用户消息中回放。
- ~~实现真正的命令面板与 `Ctrl+K`，聚合当前 Harness 的原生命令、Compact、Fork、权限和模型动作。~~ **已完成**：`palette.js` 聚合原生命令（insert/execute）、Compact、Fork、停止、刷新用量、权限模式、模型、思考强度、工作区面板与 Harness 切换；支持过滤、键盘导航与草稿态禁用标记，动作与既有菜单共用生效路径。
- ~~让首页四张卡片进入对应真实流程，而不是只聚焦输入框。~~ **已完成**：项目卡片→目录选择器；流式对话→聚焦起草；原生命令→工作区终端；文件变更→审查面板。
- ~~完成搜索、分享、语音等入口的产品边界；暂不实现的入口应隐藏或明确标为不可用。~~ **已完成**：搜索入口打通 `CommandPalette` 支持全量会话/操作实时模糊搜索；分享按钮支持一键将任务完整对话导出为 Markdown 并复制到剪贴板；设置按钮接通环境与快捷键设置抽屉面板；规划中入口标注并提供明确友好引导；语音入口标注规划中边界。
- ~~补齐项目菜单与任务菜单的键盘导航、焦点恢复和加载状态。~~ **已完成**：项目与任务列表支持上下键无障碍键盘导航与 Enter 快速激活；操作菜单关闭后严格恢复触发按钮焦点；引入轻量高性能 `Anime.js` / `GSAP` 兼容动效引擎，彻底消除弹窗、抽屉、下拉菜单与 Toast 的生硬弹出感，带来丝滑物理弹簧与错峰微交互。
...
- 默认启动已改为本项目自研自建的 Launcher / Shim / Host / Renderer 扩展（无上游 npm 运行时依赖，基于本仓 HostRuntime/ProtocolCore）；旧工作台入口为 `npm run start:workbench`。
- Pi、Claude Code、DeepSeek Harness 已完成真实 Shim 协议回复验证。DSH 原生模式使用项目内 `0.1.2-rc.1`，保留旧源码目录。
- 原生桌面完整重启后的窗口交互还需验收；启动器会重启当前 Codex Desktop，不能把协议 E2E 等同于桌面 UI 验收。
- 旧工作台会话未自动迁移到原生 Codex 会话。详见 `docs/native-codex.md`。
