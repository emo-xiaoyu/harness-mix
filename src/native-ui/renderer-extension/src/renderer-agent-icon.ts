...
function buildSvgMark(
  glyphs: readonly AgentGlyph[],
  fill: string,
  size: number,
  ...
  for (const glyph of glyphs) {
    ...
    if (glyph.fill) path.setAttribute("fill", glyph.fill);
    svg.append(path);
  }
  return svg;
}

function buildImageMark(
  source: string,
  size: number,
  ownerDocument: Document,
  rounded = false,
  ...
  if (rounded) image.style.borderRadius = "22.37%";
...
export function createRendererAgentIcon(
  ...
  const embedded = projectIcon("harnesses", agent, size, ownerDocument);
  if (embedded) return embedded;
  switch (agent) {
    case "codex":
      return buildImageMark(codexAgentIconUrl, size, ownerDocument);
    case "pi":
      return buildSvgMark(PI_PATHS, "currentColor", size, ownerDocument);
    case "claude-code":
      return buildSvgMark([{ d: CLAUDE_PATH }], "#d97757", size, ownerDocument);
    ...
    case "opencode":
      return buildImageMark(openCodeAgentIconUrl, size, ownerDocument);
    case "omp":
      return buildImageMark(ompAgentIconUrl, size, ownerDocument, true);
    case "antigravity":
