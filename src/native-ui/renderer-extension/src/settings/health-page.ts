...
// Settings → Health center: host runtime status, per-Harness handshake
// results, storage summary and crash reports. Read-only snapshot plus an
// on-demand re-probe; credentials and native sessions never pass through.
...
// Right-aligned metric block: muted label above a tabular value, optionally
// tinted by status color.
...
// Two-column row — left (icon + name + tags) and a right-aligned status —
// chained by divider borders into a list card.
...
function harnessHandshakeRow(
  document: Document,
  harness: RendererHealthHarness,
  zh: boolean,
): HTMLElement {
  ...
  return listRow(document, left, status);
}

export function createHealthSettingsPage(
  messages: RendererSettingsMessages,
  getClient: () => RendererHealthClient | null,
  ...
  return Object.freeze({
    ...
    mount(context: RendererSettingsPageMountContext) {
      ...
      const render = (snapshot: RendererHealthSnapshot): void => {
        renderRuntime(panel, snapshot);
        renderHandshakes(harnessList, snapshot);
        renderCrashReports(crashPanel, snapshot);
      };

      const renderRuntime = (target: HTMLElement, snapshot: RendererHealthSnapshot): void => {
        const runtime = snapshot.runtime;
        const instance = runtime.instance;
        const heartbeatOk = instance?.heartbeatAgeMs != null && instance.heartbeatAgeMs < STALE_HEARTBEAT_MS;

        target.replaceChildren();
        target.append(cardTitle(document, zh ? "运行时" : "Runtime"));
      ...
      const renderHandshakes = (target: HTMLElement, snapshot: RendererHealthSnapshot): void => {
        target.replaceChildren();
        target.append(cardTitle(document, zh ? "Harness 握手状态" : "Harness handshake status"));
        for (const harness of snapshot.harnesses) {
          target.append(harnessHandshakeRow(document, harness, zh));
        }
      };

      const renderCrashReports = (target: HTMLElement, snapshot: RendererHealthSnapshot): void => {
        target.replaceChildren();
        target.append(cardTitle(document, zh ? "崩溃报告（最近 10 条）" : "Crash reports (latest 10)"));
