...
// Settings → Usage center: read-only aggregation of token / spend / credits
// history across every Harness. All numbers come from the host's
// usage-history (baseline-deduped positive deltas).
...
// Right-aligned metric block: muted label on top, tabular value below; the
// aggregate panel composes these into a dashboard-style summary strip.
...
// Two-column row — left (swatch + brand icon + name) and a right-aligned
// metric — chained by divider borders into a list card.
...
function harnessLabelLeft(
  document: Document,
  harness: RendererUsageHarnessRow,
  index: number,
): HTMLElement {
    ...
    mount(context: RendererSettingsPageMountContext) {
      ...
      const renderSummary = (summary: RendererUsageSummaryResult): void => {
        ...
        for (const [index, harness] of summary.byHarness.entries()) {
          ...
          panel.append(listRow(document, harnessLabelLeft(document, harness, index), right));
        }
      };
      ...
      const renderChart = (history: RendererUsageHistoryResult): void => {
        ...
        // Assign colors by first-appearance order and remember display names.
        ...
        // Chart plot: faint gridlines over a bottom baseline, stacked bars
        // on top; hover tooltips stay on the day columns.
        ...
        for (const day of history.days) {
          const dayTotal = day.harnesses.reduce((sum, harness) => sum + (harness.totalTokens || 0), 0);
        ...
        // Legend: swatch + brand icon + name, one entry per bar color.
