...
// Human-readable byte counts: B until 1 KiB, then the largest binary unit
// where the value still fits, one decimal below ten and none above.
...
export function createStorageSettingsPage(
  messages: RendererSettingsMessages,
  getClient: () => RendererStorageClient | null,
): RendererSettingsPageDefinition {
  const zh = messages.locale === "zh-CN";
  return Object.freeze({
    id: "storage",
    label: zh ? "存储与验证" : "Storage & verification",
    icon: "storage",
    mount(context: RendererSettingsPageMountContext) {
      const document = context.content.ownerDocument;
      const heading = document.createElement("div");
      heading.className = "settings-section-label";
      heading.textContent = zh ? "会话存储" : "Session storage";

      const storage = document.createElement("section");
      storage.className = "settings-storage-block";
      ...
      optimize.className = "settings-command-button";
      ...
      const render = (value: RendererStorageInspection): void => {
        ...
        title.textContent = `Schema v${value.storageSchemaVersion}`;
      ...
      if (!client) {
        panel.textContent = zh ? "当前 Host 不支持存储治理。" : "Storage governance is unavailable on this Host.";
        optimize.disabled = true;
      } else {
        void context.runLatest(() => client.inspectStorage(), {
          success: render,
          failure(error) {
      ...
      gate.className = "settings-about-panel";
      const gateTitle = document.createElement("strong");
      gateTitle.textContent = zh ? "任务验证门禁" : "Task verification gates";
      const gateCopy = document.createElement("p");
      gateCopy.textContent = zh
