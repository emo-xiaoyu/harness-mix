...
function dataDirectory(env = process.env, platform = process.platform, home = os.homedir()) {
  if (env.CODEXHOST_DATA_DIR) return env.CODEXHOST_DATA_DIR;
  const base = platform === 'win32' ? env.APPDATA || home
  ...
  return path.join(base, 'harness-mix', 'codexhost');
}
...
function inspectPosix(env = process.env, platform = process.platform, run = execFileSync) {
  ...
  let stock = env.CODEXHOST_STOCK_CODEX_PATH;
...
module.exports = { dataDirectory, executableName, inspectPosix, assertDesktopStopped };
