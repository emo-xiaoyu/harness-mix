...
function piFamily({ id, name, icon, bin, packageHint, aliases }) {
  ...
  function spawnProcess(args, cwd, hooks, collaboration) {
    if (collaboration) args.push('--extension', path.join(__dirname, '../host/collaboration-pi.mjs'));
    const { command, args: cliArgs } = cliSpawn(bin, ["--mode", "rpc", ...args]);
    return new JsonlProcess(command, cliArgs, { cwd, ...(collaboration ? { env: { ...process.env, ...collaboration.env } } : {}) }, hooks);
  }
    ...
    return {
      ...
      async inspect() {
        ...
        return result.ok
          ...
          : { available: false, detail: `未找到 ${bin} CLI（${packageHint}）` };
      },
      ...
      async listCommands(session) {
        ...
        return [{ id: 'compact', label: '压缩上下文', description: `由 ${name} 原生总结并压缩当前会话`, action: 'execute' },
      ...
      async setModel(session, model) {
        ...
        return data ? { id: data.id ?? model.id, name: data.name ?? model.name, provider: data.provider ?? model.provider } : model;
      },
      ...
      async describe() {
        const process = spawnProcess(["--no-session"], os.tmpdir(), {});
        try {
      ...
      async fork(sourceThread, { emit: emitEvent, diagnostic, message }) {
        const process = spawnProcess(["--fork", sourceThread.nativeSessionId, "--name", `${sanitizeName(sourceThread.title)} · Fork`], sourceThread.cwd, {
          onEvent: event => forwardEvent(process, event, emitEvent),
          onDiagnostic: (message) => diagnostic(message),
        });
        try {
