...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await run(`for(let i=0;i<30&&!$('[data-thread="ui-thread"]');i++)await wait();$('[data-thread="ui-thread"]').click();for(let i=0;i<30&&!$('.activity');i++)await wait();check(!$('.activity').open,'live activity collapsed');check($('.progress-message').checkVisibility(),'progress text visible');check(!$('.reasoning-text').checkVisibility(),'reasoning details hidden');check(!$('[data-tab="__draft"]'),'no draft tab');$('.activity summary').click();check($('.activity').open&&$('.reasoning-text').checkVisibility(),'thinking expands');$('[data-live-review]').click();await wait();check($('.wb-code .add'),'live real diff');check($('.wb-undo').disabled,'live undo disabled');check($('.wb-tree-file').textContent.includes('app.js'),'changed file list');document.activeElement.blur();`);
    ...
    await run(`check($('.activity').open,'activity disclosure survives refresh');$('.wb-close').click();$('#toggleSide').click();for(let i=0;i<30&&!$('.wb-code .add');i++)await wait();check($('.wb-code .add'),'live panel reopens');`);
    ...
    await run(`$('.activity[open] summary')?.click();`);
    ...
    await run(`for(let i=0;i<30&&!$('[data-thread="ui-thread"]');i++)await wait();$('[data-thread="ui-thread"]').click();for(let i=0;i<30&&!$('.turn-history');i++)await wait();check(!$('.turn-history').open,'process collapsed');check($('.turn-history').textContent.includes('2分 33秒'),'total duration above final');check($('.final-answer').textContent.includes('已完成'),'final visible');check($('.change-card'),'change card');$('.turn-history summary').click();check($('.turn-history').open,'process expands');$('.turn-history summary').click();$('.change-heading button').click();await wait();check($('.workbench').hidden===false,'review panel opens');check($('.wb-code .add'),'added lines');check($('.wb-code .remove'),'removed lines');check(document.documentElement.scrollWidth<=innerWidth,'no overflow');`);
    await fs.mkdir('output/playwright', { recursive: true });
