// 真实 OpenClaw 链路自检（经本机 Gateway 发起一次最小 agent turn；Gateway 未运行时自动拉起）
const { HostRuntime } = require("../src/main/host/runtime");
const os = require("node:os");
const path = require("node:path");

(async () => {
  const rt = new HostRuntime({ dataDirectory: path.join(os.tmpdir(), "hm-e2e-openclaw") });
  rt.subscribe((e) => { if (e.type === "toast") console.log("  [toast]", e.text); });
  await rt.initialize();
  const thread = await rt.createThread({ harnessId: "openclaw", cwd: "E:\\harness-mix", title: "E2E 自检" });
